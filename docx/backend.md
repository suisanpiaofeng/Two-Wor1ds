...
- **描述**: 返回未读通知数
- **补充**: 该接口仍可独立使用；当前前端底部消息标签主要基于全局通知列表 + 会话列表的未读聚合刷新
...
- 已补充会话删除接口，支持删除聊天框并级联清理会话消息。
- 已补充头像字段扩容，`users.avatar_seed` 改为 `TEXT`，兼容上传后的图片数据。
...
- **游客收不到聊天消息**: 已修复前端刷新时误清游客登录态的问题，并配合前端轮询保证游客与注册账号都能收到新消息。
- **底部消息徽章不即时刷新**: 已改为由前端全局定时刷新通知与会话未读状态，不必先进入消息页。
